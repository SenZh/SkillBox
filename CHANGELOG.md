...
### 🔄 仓库定时更新与状态同步优化
- **前端实时状态自动同步**：
  - 切换到「⚙️ 仓库与配置」页面时自动重新获取最新配置与更新时间，避免长时间挂起页面停留在旧快照；
  - 窗口重新聚焦（`focus`）或切回前台时，自动静默同步仓库最新状态；
  - 页面处于前台且停留在配置页时，每 30 秒后台静默轮询一次，保证定时拉取时间实时更新。
- **定时调度日志持久化**：
  - 将后台调度器的控制台输出接入日志系统（`skillbox.log`），自动更新触发、成功、失败均留痕落盘；
  - 调度器主循环异常增加错误日志记录，杜绝异常静默吞没。
...
[0.2.1]: https://github.com/SenZh/SkillBox/releases/tag/v0.2.1
