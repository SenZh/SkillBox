...
- [x] **跨平台桌面托盘应用**：基于 pywebview 原生 WebView 窗口 + pystray 系统托盘常驻；
- [x] **关闭即缩托盘**：点关闭窗口静默隐藏到右下角托盘，服务继续后台运行；
- [x] **内嵌托管服务**：托盘应用内部托管 HTTP 服务，不再依赖独立隐藏守护进程；
- [x] **托盘右键菜单**：打开控制台 / 浏览器打开 / 同步更新 / 查看日志 / 打开目录 / 退出；
- [x] **单实例保证**：基于命名互斥体/flock 的跨进程单实例锁，重复启动改为唤醒已有窗口；
- [x] **启动自动检测并安装 CLI**：未注册 `skillbox` 命令时自动写入用户 PATH；
- [x] **开机静默自启**：开机自启入口切换为拉起桌面托盘应用（`--startup` 静默进托盘）；
- [x] **单文件 exe 打包**：`tools/build_exe.py` 产出内嵌图标的 `dist/SkillBox.exe`；
- [x] **桌面快捷方式**：`skillbox shortcut` 优先指向打包好的 exe；
- [x] **Release 成果物**：`tools/build_release.py` 产出免安装 zip + Inno Setup 安装包；
- [x] **WebUI 精简优化**：移除 Tag 下拉/快捷过滤，挂载状态改 Tab，放大搜索框；
- [x] **依赖降级兼容**：无 pywebview/pystray 时自动回退浏览器窗口模式。
